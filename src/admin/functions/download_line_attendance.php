...
foreach ($chunks as $chunk) {
    ...
    $totalAttendance += $chunkTotal;

    $groupedRows[] = [
        'label' => date('F d', strtotime($chunkStart)) . ' - ' . date('F d, Y', strtotime($chunkEnd)),
        'total' => $chunkTotal,
    ];
}
...
$src = 'data:image/jpg;base64,' . base64_encode(file_get_contents($imagePath));
...
<div class="header">
    <img src="' . $src . '" class="logo">
    <h1>Attendance Analytics Report</h1>
    <p>Monthly Attendance Summary (5-Day Groups)</p>
    <p>' . date('F Y', strtotime($month)) . '</p>
</div>

<div class="summary">
    <strong>Total Attendance Records:</strong> ' . $totalAttendance . '
</div>
